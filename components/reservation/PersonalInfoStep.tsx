...
// Import supprimé - utilisation de l'API route à la place
...
export default function PersonalInfoStep({ language, onSubmit, onBack, initialData = {}, selectedPack }: PersonalInfoStepProps) {
  ...
  const texts = {
    fr: {
      ...
      errorSending: 'Erreur lors de l\'envoi. Veuillez réessayer.'
    },
    en: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Envoyer les emails
      const result = await sendReservationEmails(reservationData);

      if (result.success) {
        // Passer les données avec le numéro de commande
        onSubmit({
          ...reservationData,
        ...
        setErrors({ general: texts[language].errorSending });
      }
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex gap-4 pt-4">
          ...
          <button
            ...
            className="flex-1 py-3 bg-[#F2431E] text-white rounded-xl font-semibold hover:bg-[#E63A1A] transition-colors disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap flex items-center justify-center"
          >
            {isSubmitting ? (
              <>
