...
export async function POST(req: NextRequest) {
  try {
    ...
    // Mettre à jour via supabaseAdmin (service role) - remettre à null
    ...
    if (updateError) {
      console.error('Erreur refus demande pro:', updateError);
      return NextResponse.json(
        { success: false, error: updateError.message || 'Erreur lors du refus' },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'Demande Pro refusée avec succès',
    });
  } catch (error: any) {
    console.error('Erreur API reject pro:', error);
    return NextResponse.json(
