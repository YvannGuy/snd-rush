...
export async function POST(req: NextRequest) {
  try {
    ...
    if (updateError) {
      console.error('Erreur activation pro:', updateError);
      return NextResponse.json(
        { success: false, error: updateError.message || 'Erreur lors de l\'activation' },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'Accès Pro activé avec succès',
    });
  } catch (error: any) {
    console.error('Erreur API activate pro:', error);
    return NextResponse.json(
