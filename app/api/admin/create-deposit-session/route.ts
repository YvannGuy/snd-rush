...
export async function GET(req: NextRequest) {
  try {
    ...
    // Convertir le montant de la caution (en centimes depuis l'URL)
    const depositAmount = parseInt(depositParam, 10);

    if (isNaN(depositAmount) || depositAmount <= 0) {
      // Si la caution est 0 ou invalide, rediriger vers le dashboard admin
      return NextResponse.redirect(
        new URL('/admin/paiement?success=true', process.env.NEXT_PUBLIC_BASE_URL!)
      );
    }
    ...
    if (!customerId && (mainSession.customer_email || customerEmail)) {
      const customer = await stripe.customers.create({
        email: mainSession.customer_email || customerEmail || undefined,
      });
    ...
    const depositSession = await stripe.checkout.sessions.create({
      ...
      payment_intent_data: {
        ...
        metadata: {
          ...
          customerEmail: customerEmail || mainSession.customer_email || '',
        },
      },
      custom_text: {
        submit: {
